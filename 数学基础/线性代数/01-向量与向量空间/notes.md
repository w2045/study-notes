...
**证明**：$\mathbf{0} \in U \cap W$；$\mathbf{x}, \mathbf{y} \in U \cap W \implies \mathbf{x}+\mathbf{y}$ 同时在 $U$ 和 $W$ 中；标量乘法同理。$\square$
...
class Vector:
    ...
    # ─── 标准基向量 ───
    @staticmethod
    def e(i: int, dim: int) -> Vector:
        """第 i 个标准基向量 (1-indexed)。"""
        v = Vector.zero(dim)
        v._data[i - 1] = 1.0
        return v
...
```python
# 标准基
e1, e2, e3 = Vector.e(1, 3), Vector.e(2, 3), Vector.e(3, 3)
...
$\mathbf{a} = \mathbf{a} + \mathbf{0} = \mathbf{a} + (\mathbf{v} + \mathbf{b}) = (\mathbf{a} + \mathbf{v}) + \mathbf{b} = \mathbf{0} + \mathbf{b} = \mathbf{b}$。$\square$

### 12.3 标量 0 产生零向量

> **定理**：$0 \cdot \mathbf{v} = \mathbf{0}$。

**证明**：$0 \cdot \mathbf{v} = (0+0)\mathbf{v} = 0\mathbf{v} + 0\mathbf{v}$。两边加 $-0\mathbf{v}$ 得 $0\mathbf{v} = \mathbf{0}$。$\square$
...
**证明**：设 $\sum_{i=1}^{k+1} c_i\mathbf{v}_i = \mathbf{0}$。若 $c_{k+1} \neq 0$，则 $\mathbf{v}_{k+1} = -\frac{1}{c_{k+1}}\sum_{i=1}^{k} c_i \mathbf{v}_i \in \operatorname{span}$，矛盾。故 $c_{k+1} = 0$，归约到前 $k$ 个的线性无关性，得全零解。$\square$
