...
上面手动调用 `iter()` 和 `next()` 展示了迭代的底层机制。但没人愿意在代码里写 `try/except StopIteration`——生成器让你用熟悉的 `def` 语法创建迭代器，把暂停和恢复逻辑隐藏在 `yield` 关键字背后。
...
`yield` 让创建迭代器变得简单，但它隐藏了底层机制。了解迭代器协议本身——`__iter__` 返回自身，`__next__` 逐个产出——能让你理解生成器实际做了什么，在需要精确控制状态时也可以手动实现。
...
手动实现一次迭代器协议让你看清原理。日常编程中，大多数迭代需求已经有现成工具——Python 内置和 `itertools` 模块覆盖了计数、分组、组合、切片等常见模式。
...
以上工具各自解决一个独立问题。它们真正的威力在于**串联**——一个生成器的输出直接喂给下一个，形成一条惰性处理流水线。数据像水流过管道，每次只处理当前元素，全程不把整个数据集加载到内存。
