...
现在我们有了一套判断真假的工具——比较运算给出 `True`/`False`，逻辑运算组合多个条件，真值测试让任何值都能当条件用。但光有判断还不够——我们要让程序根据判断结果**走不同的路径**。这就是 `if` 语句。
...
`if` 让你**选择**一条路径走一次。但程序中还有一种同样常见的需求：**重复**执行一段代码，直到条件不满足。比如「只要余额大于 0，就继续扣款」或者「不断读取数据直到文件末尾」。这就是循环，我们从 `while` 开始。
...
`while` 的问题在于：你需要自己管理三件事——初始化计数器（`i = 1`）、更新计数器（`i += 1`）、检查终止条件（`i <= n`）。忘掉任何一步就是死循环或 off-by-one 错误。当我们要做的只是「遍历一个序列的每个元素」时，`while` 的这三步纯属重复劳动。`for` 循环替我们管理这些细节：
...
`for` 和 `while` 默认的规则是「把循环体完整跑完」。但实际场景中常有例外：遍历列表找某个元素——找到了就**不用继续**了（break）；遍历处理数据——遇到损坏的行就**跳过这一条**继续处理下一条（continue）。这两种中途干预是循环控制不可或缺的能力。
...
上面我们一直在用 `if` **语句**——它控制程序走哪条分支，本身不产生值。但第一章 §6 提到过一个更简洁的工具：`x if cond else y` 是一个**表达式**，它直接求值为一个值，可以嵌在任何需要值的地方。这里快速回顾，因为它和 `if` 语句是同一逻辑的两种形态：

```python
x = "偶数" if n % 2 == 0 else "奇数"
```

这是**表达式**（返回值），不同于 `if` **语句**（只执行动作）。第二章后面当你写 `filter`、`map` 等高阶函数时，条件表达式会频繁出现——lambda 里不能写 `if` 语句，但可以写条件表达式。

---

## 9. 环境模型中的控制流

本章学完了条件判断和循环的语法。最后回到第一章 §5 建立的环境模型，回答一个重要问题：`if`/`while`/`for` 在执行时创建新帧吗？不创建。它们和函数调用的关键区别在于——函数调用创建一个隔离的局部环境，而控制流语句只是在**当前帧内**按条件选择或重复执行代码。
