...
`Dog` 类的例子展示了类的表面语法——`class`、`__init__`、`self`。现在深入两个核心机制：`buddy.bark()` 调用时 `self` 究竟怎么传到 `bark` 的？以及 `buddy.name` 到底经过怎样的查找链找到值？
...
属性查找链揭示了方法如何被找到。但类中可以定义不同类型的方法——最常用的是实例方法，但 Python 还有两种不依赖实例的方法变体，各自有不同的用途。
...
上面的方法都需要显式调用（`v1 + v2` 是运算符，但 `v1.x` 是直接属性访问）。有时你希望在属性被访问或赋值时自动执行逻辑——比如在赋值时校验数据的合法性，或在访问时动态计算。`property` 让你把方法伪装成属性，保留 `obj.attr` 的简洁语法，同时有方法的能力。
...
`property` 控制的是**属性访问**时发生什么。还有两个装饰器控制的是**方法调用**的绑定规则——方法默认绑定到实例（`self`），但有时你需要绑定到类本身（`cls`），或完全不绑定。
