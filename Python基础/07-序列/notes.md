...
列表灵活可变，适合「收集和修改数据」的场景。但很多时候你只需要**存储一段文字，不希望它被意外改动**——这就是字符串，Python 中最常用的不可变序列。
...
上面逐一讲了四种序列。现在反过来看——它们有哪些**共同能力**？Python 为所有序列类型提供了统一的接口。掌握这些通用操作后，你不需要为每种类型记不同的 API。
...
`zip`、`enumerate`、`min`、`sum` 解决的是序列的「读」问题。但有另一种需求更常见——「对每个元素做同样的变换，生成一个新序列」。可以写一个 `for` 循环加 `append`，但 Python 提供了更简洁的语法。
...
列表推导式创建**新**列表，拷贝的是元素引用。但如果你有嵌套列表（列表的列表），拷贝外层后，内层列表是共享还是独立？这引出一个跨越本章和第九章（可变性）的重要概念——浅复制和深复制。
