...
环境图也揭示了递归的一个代价：`factorial(4)` 创建了 5 个帧，占用 O(n) 的栈空间。而第一章的循环版本只用一个帧、两个变量。既然循环更省空间，为什么要用递归？答案在下一节。
...
上面所有模式都是一个函数自己调用自己。但递归不需要局限于自己——两个函数可以互相调用，形成循环依赖。这听起来像死循环，但只要有基本情况终止，它就是合法且有用的。
...
相互递归说明递归调用可以出现在函数体的任何位置。有一个特殊情况值得单独讨论：当递归调用是函数体**最后一步**，返回后不需要额外计算时——这就是尾递归，它在理论上有重要的优化可能。
