...
有理数例子演示了数据抽象的核心机制。从它身上可以提炼出一个通用模式——任何抽象数据类型都由三类操作构成。
...
有理数的抽象只有一层：一个有理数就是一个对子。但数据抽象同样适用于**递归结构**——第五章和第六章的递归思想在这里和数据抽象汇合。树就是最好的例子：每个节点本身是一个树，包含若干子树。
...
到目前为止，所有 ADT 底层都用了列表（`[numer, denom]`、`[label, branches]`）。一个有趣的问题：如果没有任何数据结构，**纯用函数**能表示数据吗？答案是能——而且这恰好演示了第三章闭包和本章数据抽象的结合。
